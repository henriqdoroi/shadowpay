    ...
    intervalRef.current = setInterval(async () => {
      try {
        const txRes = await axios.get(
          `https://api.safira.cash/api/payments/public/transaction/${saleId}`
        );
        ...
        await axios.put(
          `https://api.safira.cash/api/sales/${saleId}`,
          { status: "approved" },
          { headers: { Authorization: `Bearer ${token}` } }
        );

        const productRes = await axios.get(
          `https://api.safira.cash/api/products/${productId}/checkout`
        );
  ...
  const handleCreateSale = async ({
    ...
    try {
      ...
      const response = await axios.post(
        "https://api.safira.cash/api/sales/public",
        {
      ...
      const timer = setTimeout(async () => {
        try {
          const saleRes = await axios.get(
            `https://api.safira.cash/api/sales/public/${saleId}`
          );
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const url = token
          ? `https://api.safira.cash/api/products/${id}`
          : `https://api.safira.cash/api/products/${id}/checkout`;
        ...
        if (savedSaleId && utmifyTokenRef.current) {
          try {
            const txRes = await axios.get(
              `https://api.safira.cash/api/admin/transactions?saleIds=${savedSaleId}`,
              { headers: { Authorization: `Bearer ${token}` } }
            );
